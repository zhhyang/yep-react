...
class Popover extends React.PureComponent<PopoverProps,any> {
  ...
  closeChildOpenedPopover() {
    const childOpenedPopover = this.popoverContent && this.popoverContent[Popover.OPENED_POPOVER];
  ...
  renderContent() {
    ...
    this.renderContent = () => {
      const {triggerMode, content, open, onToggle, shouldOpen, shouldClose, disabled, aligned, ...other} = this.props;
      other.onClick = e => {
        e.stopPropagation();
        this.closeChildOpenedPopover();
      };
